...
from gwinferno.interpolation import LogXLogYBSpline, LogYBSpline, LogXBSpline, BSpline
...
def model(mass_model, spin_models, z_model, pedict, injdict, total_inj, Nobs, Tobs, sample_prior=False):
    ...
    mag_cs = numpyro.sample('mag_cs', dist.Normal(0,2), sample_shape=(mag_knots,2))
    mag_tau = numpyro.sample("mag_tau",  dist.Uniform(1,10))
    numpyro.factor("mag1_log_smoothing_penalty", calculate_penalty(mag_cs[:,0], mag_tau, degree=2))
    numpyro.factor("mag2_log_smoothing_penalty", calculate_penalty(mag_cs[:,1], mag_tau, degree=2))
    tilt_cs = numpyro.sample('tilt_cs', dist.Normal(0,2), sample_shape=(tilt_knots,2))
    tilt_tau = numpyro.sample("tilt_tau",  dist.Uniform(1,10))
    numpyro.factor("tilt1_log_smoothing_penalty", calculate_penalty(tilt_cs[:,0], tilt_tau, degree=2))
    numpyro.factor("tilt2_log_smoothing_penalty", calculate_penalty(tilt_cs[:,1], tilt_tau, degree=2))
...
def main():
    args = load_parser()
    label=label=f'{args.outdir}/bsplines_{args.mass_knots}m1_{args.q_knots}q_ind{args.mag_knots}mag_ind{args.tilt_knots}tilt_pl{args.z_knots}z'
