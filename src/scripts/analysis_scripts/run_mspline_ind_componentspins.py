...
#plt.rcParams['plot.max_subplots'] = 35
...
def model(mass_dict, spin_dict, z_dict, prior_dict, total_inj, Nobs, Tobs, args):
    ...
    if args.dirichlet:
        mag_pcs = numpyro.sample('mag_pcs', dist.Dirichlet(jnp.ones(spin_knots)))
        tilt_pcs = numpyro.sample('tilt_pcs', dist.Dirichlet(jnp.ones(spin_knots)))
        mag_scs = numpyro.sample('mag_scs', dist.Dirichlet(jnp.ones(spin_knots)))
        tilt_scs = numpyro.sample('tilt_scs', dist.Dirichlet(jnp.ones(spin_knots)))
    else:
...
def main():
    args = setup_parser()
    if args.dirichlet:
        label = f'results/paper/mspline_{args.mass_knots}m1_{args.spin_knots}ind_compspins_dirchprior_powerlaw_q_z'
    else:
    ...
    if os.path.isfile(f'{label}_posterior_samples.h5'):
        print("*********LOADING SAVED POSTERIOR SAMPLES********")
    ...
    if args.dirichlet:
        ppmags, psmags, mags = calculate_ind_spin_ppds(posterior['mag_pcs'], posterior['mag_scs'], posterior['rate'], MSplineIndependentSpinMagnitudes, nknots=args.spin_knots, xmin=0,xmax=1)
        pptilts, pstilts, tilts = calculate_ind_spin_ppds(posterior['tilt_pcs'], posterior['tilt_scs'], posterior['rate'], MSplineIndependentSpinTilts, nknots=args.spin_knots, xmin=-1,xmax=1)
    else:
