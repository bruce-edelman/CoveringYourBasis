...
, and a difference order of $r$ in our smoothing penalty (see \bruce{cite P-splines paper}) is defined as:

\begin{eqnarray}
\tau_\lambda \propto \Gamma(a, b) \\
...
\begin{align}
    \bm{p} \propto \mathcal{D}(m) \\
    p(\tau_\lambda | \bm{p}) = \sum_i^m p_i \Gamma(1, b_m)
\end{align}
...
and $b_m$ is an array of $m$ different b shape parameters for the Gamma distribution.
...
at hard parameter boundaries but this removes the feature that with equal weights the spline function is flat. We follow the distinction from \bruce{CITE psplines} 
