...
Spline functions have been shown to be sensitivite to the chosen number of knots, and their locations or spacing \citep{deBoor78}. 
...
to the likelihood based on the difference of adjacent knot coeficients \citep{eilers2021practical}. The linear combination of spline basis components 
or the resulting spline function is flat when the basis coeficients are equal (see Figure \ref{fig:spline_basis}). By penalizing the likelihood as 
the differences between adjacent knot coeficients get larger, one gets a smoothing of the spline function inferences \citep{eilers2021practical}. 
Since we are using hierarchical Bayesian inferece as our statistical framework, we formulate this penalized likelihood with its Bayesian analog. 
The difference penalty is equivalent to applying a stochastic differnece prior among the knots, as described in \citet{BayesianPSplines} and \citet{Jullion2007RobustSO}. 
This is also sometimes referred to as a Gaussian random walk prior, since each coeficient is normally distributed about its neighboring coeficients. 
...
of the b hyper-parameter for Gamma distribution prior on $\tau_\lambda$. We reduce this affect by following the method introduced in \citet{Jullion2007RobustSO}
...
n degrees of freedom, people often will place knots on top of each other at hard parameter boundaries \cite{deBoor78,monotone_regression_splines}. 
...
We follow the distinction from \citet{eilers2021practical} that the difference penalty or smooothing prior is only valid with ``proper'' spline bases. 
