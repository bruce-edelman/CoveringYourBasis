...
Figure \ref{fig:mass_distribution} shows the primary mass distribution inferred with our B-Spline model (red), where we see that it recovered consistent 
...
that can possibly get to as low as $\sim 3\msun$. Since non-parametric models cannot provide meaningful insights where there is no data - we enforce the minimum primary mass of 
...
evolution theory. This could be a sign of the significant presence of subpopulations that may avoid PISN as the binary forms, but the confirmation of their existence cannot be determined 
with current catalog sizes. 
...
We report the B-Spline model's inferred marginal mass ratio distribution in figure \ref{fig:q_distribution}, where we can begin to see some departure from a simple 
power law. Specifically there looks to be a small decrease or flattening of the merger rate near equal mass ratios. Then we see a small decline to flat plateau from 
$q\sim0.3$ to $q\sim0.7$. The last step down in rate below $q\sim0.3$ is due to the minimum mass ratio truncation defined by: $q_\mathrm{min}=\frac{m_\mathrm{min}}{m_1}$. 
When marginalizing over the primary mass distribution with a strong peak at $10\msun$, we get a strong truncation at $q\sim0.3$ which is the minimum secondary mass, $3\msun$, 
over the most common primary mass, $10\msun$. 
...
We model the spin magnitude distributions as IID B-Spline distributions as a flexible check
to the assumptions of the \textsc{Default} model. Figure \ref{fig:iid_spinmag_dist} shows the inferred spin magnitude distribution with the B-Spline model, 
compared with the \textsc{Default} model. The B-Spline model finds broad agreement with the beta distribution, both peaking near ~0.2, with 90\% 
of BBH spins below \result{$\CIPlusMinus{\macros[BSplineIIDCompSpins][a_90percentile]}$} at 90\% credibility. The regions of parameter space where they don't agree as well are at 
the boundaries. The B-Spline model has no enforcement for vanishing support at the extremal values like the Beta distribution, 
...
shows the inferred spin magnitude distribution with the B-Spline model, compared with the \textsc{Default} model from \citet{o3b_astro_dist}. 
...
and positive when aligned tilts are favored. We find a $\log_{10}Y=$\result{$\CIPlusMinus{\macros[BSplineIIDCompSpins][log10gammaFrac]}$}, exhibiting a 
...
consistent with both each other and the \textsc{Default} model's PPD. We find the two distributions to peak at: $\cos{\theta_1}=$\result{$\CIPlusMinus{\macros[BSplineIndependentCompSpins][peakCosTilt1]}$} 
...
\begin{figure}[h]
    ...
    the 90\% credible interval. We show the inferred PPD from the \textsc{PowerlawRedshift} (blue) model from the LVK's GWTC-3 population analyses \citep{o3b_astro_dist}.}
    \label{fig:rofz}
